...
class CBState(Enum):
    ...
    Modelled as an Enum to enforce strict state membership.
    """
    CLOSED    = "CLOSED"      
    OPEN      = "OPEN"        
    HALF_OPEN = "HALF-OPEN"   
...
class CircuitBreaker:
    ...
    def _on_failure(self, elapsed_ms: float, exc: Exception):
        ...
        if self._state == CBState.HALF_OPEN:
            # Probe failed → stay open and reset timer
...
if __name__ == "__main__":
    ...
    print("Circuit Breaker Demo — Full State Cycle")
    ...
    def always_fail(order_id: int):
        raise ConnectionError(f"Payment service timeout for order #{order_id}")

    def always_succeed(order_id: int):
        return {"status": "ok", "order_id": order_id, "charged": True}

    for i in range(1, 6):
        result = cb.call(always_fail, order_id=i)
        print(f"  Call {i:02d} | state={cb.state.value:9s} | result={result}")

    print(f"Phase 2: Circuit is OPEN — fallback returned immediately ──")
    for i in range(6, 9):
        result = cb.call(always_fail, order_id=i)
        print(f"  Call {i:02d} | state={cb.state.value:9s} | result={result}")

    print(f"Status after OPEN ──")
    ...
    print(f"Phase 4: HALF-OPEN — probe call sent ──")
    result = cb.call(always_succeed, order_id=99)
    print(f"  Probe | state={cb.state.value:9s} | result={result}")

    print(f"Phase 5: Back to CLOSED — normal operation ──")
    for i in range(10, 13):
        result = cb.call(always_succeed, order_id=i)
        print(f"  Call {i:02d} | state={cb.state.value:9s} | result={result}")
