"""
payment_service/grpc_server.py
gRPC server for PaymentService — wraps the existing CircuitBreaker.
Generated stubs assume you ran:
    python -m grpc_tools.protoc -I../protos --python_out=. --grpc_python_out=. ../protos/payment.proto
"""
...
# Generated stubs (run protoc first)
import payment_pb2
import payment_pb2_grpc

# Reuse the CircuitBreaker from Siham's earlier work
import sys, os
...
_crashed = False          # toggled by /crash and /recover RPCs
_crash_lock = threading.Lock()


def _payment_logic(order_id: str, amount: float, customer_id: str) -> dict:
    """
    Core payment processing — raises on failure so the CB can track it.
    Simulates a ~20 % random failure rate plus a fixed latency.
    """
    with _crash_lock:
        if _crashed:
            raise ConnectionError("Payment processor is DOWN (forced crash)")

    # Simulate latency (50–200 ms)
...
# One CB instance shared across all RPCs
_cb = CircuitBreaker(
    name="payment-cb",
    failure_threshold=5,
...
# ─── Helper: convert repeated VectorClockEntry ↔ dict ───────────────────────
...
class PaymentServicer(payment_pb2_grpc.PaymentServiceServicer):
    ...
    def Compute(self, request, context):
        ...
        # Echo back a bumped vector clock (payment_service counter + 1)
